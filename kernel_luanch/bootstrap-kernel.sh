...
KERNEL_SHELL_PORT=${KERNEL_SHELL_PORT}
KERNEL_IOPUB_PORT=${KERNEL_IOPUB_PORT}
KERNEL_STDIN_PORT=${KERNEL_STDIN_PORT}
KERNEL_HB_PORT=${KERNEL_HB_PORT}
KERNEL_CONTROL_PORT=${KERNEL_CONTROL_PORT}
KERNEL_COMM_SOCKET_PORT=${KERNEL_COMM_SOCKET_PORT}
...
launch_python_kernel() {
  ...
  then
	...
	python ${KERNEL_LAUNCHERS_DIR}/python/scripts/launch_ipykernel.py --kernel-id ${KERNEL_ID} \
	      ...
	      --kernel_shell-port ${KERNEL_SHELL_PORT} \
	      --kernel_iopub-port ${KERNEL_IOPUB_PORT} \
	      --kernel_stdin-port ${KERNEL_STDIN_PORT} \
	      --kernel_hb-port ${KERNEL_HB_PORT} \
        --kernel_control-port ${KERNEL_CONTROL_PORT} \
        -- kernel_comm_socket_port ${KERNEL_COMM_SOCKET_PORT} \
        --${kernel_class_option}
	{ set +x; } 2>/dev/null
}
